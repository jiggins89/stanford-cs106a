...
public class Breakout extends GraphicsProgram {
	...
	private void addTurnsLabel(){
		GLabel turns = new GLabel ("Lives left: "+NTURNS);
		turns.setFont ("Time-15");
		add (turns, 10,25);
	...
	private void gameOverLabel(){
		...
		loser.setFont ("Times-Bold-50");
		loser.setColor (Color.PINK);
		add (loser, (WIDTH / 2 - (loser.getWidth()/2)), (HEIGHT  / 2 + loser.getAscent()) );
	}
	...
	private GObject getCollidingObject(){
		GObject xy = getElementAt (ball.getX() + (BALL_RADIUS*2), ball.getY() + (BALL_RADIUS*2));			
		return xy;
	}
